...
exports.executeCodeContainer = async (spawn, path, codeLanguage) => {
    ...
    try {
        return await new Promise((res, rej) => {
            if (codeLanguage === "c") {
                ...
                const dockercomposeProcess = spawn('docker', ['compose', 'run', '--rm', 'c-compiler'], // remove container after it runs
                { cwd: cDockerDir });
                ...
                const dockercomposeProcess = spawn('docker', ['compose', 'run', '--rm', 'js-compiler'], // remove container after it runs
                { cwd: jsDockerDir });
                dockercomposeProcess.on('error', (err) => {
                    rej(err);
                });
                ...
                dockercomposeProcess.on('close', (code) => {
                    ...
                    if (code !== 0) {
                        ...
                        const caseArrayArray = casestringArray
                            .map((string) => {
                            return string
                                .split("\n")
                                .filter((item) => {
...
//# sourceMappingURL=drawExecutionService.js.map
...
- Created a Monorepo setup using npm workspaces.
...
1. I had used localstorage for storing the undo-redo array which forced me to store vectors (Drawn shapes' coordinate data) instead of imageData in it - to restrict the memory usage (As Local storage maximum size is 5MB). But it reduces undo performance slightly when there are large single freehand drawings. I would improve this by storing image data in the undo-redo array- but in indexed db instead of local storage.
